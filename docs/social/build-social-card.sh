#!/usr/bin/env bash
# Render Zyvor OTA social cards with Google Chrome + macOS `sips`.
...
SOCIAL_OUT="${1:-$HERE/ota-social-card.jpg}"
SHARE_OUT="${2:-$HERE/ota-share-card.png}"

PNG="$(mktemp "${TMPDIR:-/tmp}/ota-social.XXXXXX.png")"
PNG2="$(mktemp "${TMPDIR:-/tmp}/ota-share.XXXXXX.png")"
trap 'rm -f "$PNG" "$PNG2"' EXIT

"$CHROME" --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=1 \
  --window-size=1600,900 --screenshot="$PNG" "file://$HERE/ota-social-card.html" >/dev/null 2>&1
...
"$CHROME" --headless=new --disable-gpu --hide-scrollbars --force-device-scale-factor=1 \
  --window-size=1200,630 --screenshot="$PNG2" "file://$HERE/ota-share-card.html" >/dev/null 2>&1
