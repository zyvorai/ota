...
3. Run formatting, tests/race/vet, simulator E2E, `make qualify`, and both architecture builds.
4. Run govulncheck and review RAUC/BSP image advisories separately.
5. Keep board qualification status explicit in the release notes (`docs/QUALIFICATION.md`).
6. Push a reviewed version tag when ready. The release workflow builds artifacts,
   generates a source-distribution SBOM, checksums them, signs SHA256SUMS with
   Cosign, uploads a workflow artifact, and **publishes a GitHub Release** with
   those files. It does not deploy to devices.
